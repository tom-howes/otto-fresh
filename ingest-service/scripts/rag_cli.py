...
def main():
    parser = argparse.ArgumentParser(
        description='RAG-based Code Assistant with GitHub Integration')
    ...
    try:
        if args.service == 'qa':
            ...
            was_streaming, captured_response = print_streaming_response(
                result, 'qa')
            ...
            if not was_streaming:
                ...
                if result.get('sources'):
                    ...
                    for i, src in enumerate(result['sources'], 1):
                        print(
                            f"{i}. {src['file']} (lines {src['lines']}) - {src['type']}")
            ...
            if was_streaming and captured_response:
                # POST-PROCESSING: Save and push the captured streaming
                # response
                print(f"\n{'=' * 60}")
                        ...
                        if github_result.get('success'):
                            if github_result.get('pr_url'):
                                print(
                                    ...
                                    f" {github_result['pr_url']}")
                            print(
                                f"✓ Branch: {github_result.get('branch', 'N/A')}")
                            print(
                                f"✓ File: {github_result.get('file_path', 'N/A')}")
                        else:
                            print(
                                f"❌ GitHub push failed:"
                                f" {github_result.get('error', 'Unknown error')}")
                    except Exception as e:
            ...
            if was_streaming and captured_response:
                ...
                code_content = rag._extract_code_from_response(
                    captured_response)

                # Combine with original context
                full_code = result.get(
                ...
                if args.push and args.file:
                    ...
                    try:
                        ...
                        if github_result.get('success'):
                            ...
                            if github_result.get('pr_url'):
                                print(
                                    f"✓ Pull request created: {github_result['pr_url']}")
                        else:
                            print(
                                f"❌ GitHub push failed:"
                                f" {github_result.get('error', 'Unknown error')}")
