...
def analyze_chunk_quality(repo_path, sample_size=5):
    ...
    print(
        f"  Avg content size: {sum(len(c['content']) for c in chunks) / len(chunks):.0f} chars")
    avg_enriched_size = (sum(len(c['enriched_content']) for c in chunks) / len(chunks))
    print(f"  Avg enriched size: {avg_enriched_size:.0f} chars")
    ...
    print(f"\n📋 CONTEXT COVERAGE:")
    print(
        f"  With imports: {with_imports}/{len(chunks)} ({with_imports / len(chunks) * 100:.1f}%)")
    print(
        f"  With class info: {with_classes}/{len(chunks)}"
        f" ({with_classes / len(chunks) * 100:.1f}%)")
    print(
        f"  With function info: {with_functions}/{len(chunks)}"
        f" ({with_functions / len(chunks) * 100:.1f}%)")
    print(
        f"  With embeddings: {with_embeddings}/{len(chunks)}"
        f" ({with_embeddings / len(chunks) * 100:.1f}%)")
    ...
    for i, chunk in enumerate(chunks[:sample_size], 1):
        ...
        print(
            f"Lines: {chunk['start_line']}-{chunk['end_line']} ({chunk['num_lines']} lines)")
        print(f"Language: {chunk['language']}")
        print(f"\nContext Available:")
        print(
            f"  Imports: {', '.join(chunk.get('file_imports', [])[:3]) or 'None'}")
        print(
            f"  Classes: {', '.join(chunk.get('file_classes', [])) or 'None'}")
        print(
            f"  Functions: {', '.join(chunk.get('file_functions', [])[:5]) or 'None'}")
...
def assess_documentation_readiness(chunks):
    ...
    chunks_with_context = [c for c in chunks if c.get(
        'file_imports') and len(c['content']) > 100]
    ...
    avg_enriched = sum(len(c['enriched_content'])
                       for c in chunks) / len(chunks)
    if avg_enriched > 1500:
        print(f"  ✅ GOOD: Rich context ({avg_enriched:.0f} chars avg)")
    else:
        print(
            f"  ⚠️  MODERATE: Context could be richer ({avg_enriched:.0f} chars avg)")
