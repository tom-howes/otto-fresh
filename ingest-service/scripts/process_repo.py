...
def main():
    parser = argparse.ArgumentParser(
        description='Chunk repository with enhanced context')
    parser.add_argument('repo', help='Repository path (owner/repo)')
    parser.add_argument('--chunk-size', type=int, default=150,
                        help='Lines per chunk (default: 150)')
    parser.add_argument('--overlap', type=int, default=10,
                        help='Overlap lines (default: 10)')
    parser.add_argument('--basic', action='store_true',
                        help='Use basic chunker (faster, less context)')
    ...
    parser.add_argument('--bucket-processed',
                        default=config.getenv('BUCKET_PROCESSED'))
    ...
    if args.basic:
        ...
        chunker = CodeChunker(
            args.project_id, args.bucket_raw, args.bucket_processed)
