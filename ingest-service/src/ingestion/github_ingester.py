...
class GitHubIngester:
    ...
    def _make_github_request(self, url: str, max_retries: int = 3) -> Dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(
                    f"⚠️  SSL Error (attempt {attempt + 1}/{max_retries}): {e}")
    ...
    def ingest_repository(self, repo_url: str,
        ...
        try:
            ...
            tree_url = (
                ...
                f"/git/trees/{target_branch}?recursive=1"
            )
            ...
            files_metadata = self._process_files(
                owner, repo, code_files, repo_path, target_branch)
    ...
    def _process_files(self, owner: str, repo: str, files: List[Dict],
        ...
        github_client = Github(
            self.github_token) if self.github_token else Github()
            ...
            try:
                ...
                if idx % 10 == 0 or idx == len(files):
                    print(
                        ...
                        f" ({len(files_metadata)} successful, {skipped} skipped)")
    ...
    def _save_metadata(self, repo_path: str, metadata: Dict):
        ...
        print(
            f"📊 Metadata saved: gs://{self.bucket_name}/{repo_path}/metadata.json")
