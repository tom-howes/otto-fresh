...
class EnhancedCodeChunker(CodeChunker):
        ...
        for line in content.split('\n')[:200]:  # Check first 200 lines
            ...
            if stripped.startswith('import '):
                ...
                for module in modules:
                    module = module.strip().split()[0].split('.')[
                        0]  # Get root module
    ...
    def _extract_python_decorators(self, content: str) -> List[str]:
        ...
        for line in content.split('\n'):
            stripped = line.strip()
            if stripped.startswith('@'):
                # Extract decorator name (handle @decorator and
                # @decorator(...))
    ...
    def _extract_python_globals(self, content: str) -> List[str]:
        ...
        for line in content.split('\n')[:150]:
            ...
            if '=' in line and not any(
                ...
                match = re.search(
                    r'^([A-Z_][A-Z0-9_]*|[a-z_][a-z0-9_]*)\s*=', line)
                if match:
    ...
    def _extract_python_exceptions(self, content: str) -> Dict:
        ...
        for i, line in enumerate(lines):
            ...
            if 'class ' in line and any(
                    x in line for x in ['Exception', 'Error']):
        ...
        for line in content.split('\n'):
            ...
            # Method in a class
                ...
                if match:
                    ...
                    if '@staticmethod' in content[max(
                            0, content.find(line) - 100):content.find(line)]:
                        class_methods[current_class]['static'].append(
                            method_name)
                    elif '@classmethod' in content[
                        max(0, content.find(line) - 100):content.find(line)
                    ]:
                        class_methods[current_class]['class_methods'].append(
                            method_name)
                    elif '@property' in content[
                        max(0, content.find(line) - 100):content.find(line)
                    ]:
                        class_methods[current_class]['properties'].append(
                            method_name)
                    else:
                        class_methods[current_class]['methods'].append(
                            method_name)
    ...
    def _extract_js_exports(self, content: str) -> Dict:
        ...
        for line in content.split('\n')[:200]:
            ...
            if 'export ' in line and 'default' not in line:
                match = re.search(
                    r'export\s+(?:const|let|var|function|class|interface|type)\s+(\w+)', line)
                if match:
                    name = match.group(1)
            ...
            if 'interface ' in line:
                ...
                if match:
                    ...
                    while j < min(i + 30, len(lines)):
                        ...
                        if prop_match:
                            properties.append(
                                f"{prop_match.group(1)}: {prop_match.group(2).strip()}")

                        j += 1

                    # First 10 properties
                    interfaces[interface_name] = properties[:10]
    ...
    def _build_enriched_content(self, chunk: Dict, file_path: str,
        ...
        if file_context.get('functions'):
            parts.append(
                f"# Functions: {', '.join(file_context['functions'][:12])}")

        # ===== PYTHON-SPECIFIC CONTEXT =====
        if language == 'python':
            if file_context.get('decorators'):
                parts.append(
                    f"# Decorators: {', '.join(file_context['decorators'][:8])}")

            if file_context.get('global_vars'):
                parts.append(
                    f"# Globals: {', '.join(file_context['global_vars'][:8])}")

            if file_context.get('async_functions'):
                parts.append(
                    f"# Async Functions: {', '.join(file_context['async_functions'][:6])}")

            # Exception handling
            exceptions = file_context.get('exception_handling', {})
            if exceptions.get('raised'):
                parts.append(
                    f"# Raises: {', '.join(exceptions['raised'][:5])}")
            if exceptions.get('caught'):
                parts.append(
                    f"# Catches: {', '.join(exceptions['caught'][:5])}")
            if exceptions.get('custom'):
                parts.append(
                    f"# Custom Exceptions: {', '.join(exceptions['custom'])}")
            ...
            if chunk_name in type_hints:
                ...
                if hint.get('params'):
                    params_str = ', '.join(
                        [f"{k}: {v}" for k, v in list(hint['params'].items())[:5]])
            ...
            for class_name, methods in class_methods.items():
                if class_name in chunk.get('content', ''):
                    if methods.get('methods'):
                        parts.append(
                            f"# Class {class_name} Methods: {', '.join(methods['methods'][:6])}")
                    if methods.get('static'):
                        parts.append(
                            f"# Static Methods: {', '.join(methods['static'])}")
                    if methods.get('properties'):
                        parts.append(
                            f"# Properties: {', '.join(methods['properties'])}")
        ...
        if language == 'typescript':
            ...
            if file_context.get('generics'):
                parts.append(
                    f"# Generics Used: {', '.join(file_context['generics'][:5])}")
        ...
        if language in ['javascript', 'typescript']:
            ...
            if exports.get('named'):
                parts.append(
                    f"# Named Exports: {', '.join(exports['named'][:8])}")
        ...
        if language == 'java':
            if file_context.get('annotations'):
                parts.append(
                    f"# Annotations: {', '.join(file_context['annotations'][:8])}")

            if file_context.get('interfaces'):
                parts.append(
                    f"# Interfaces: {', '.join(file_context['interfaces'][:6])}")
