...
class CodeChunker:
    ...
    def __init__(self, project_id: str, bucket_raw: str,
    ...
    def process_repository(self, repo_path: str) -> List[Dict]:
        ...
        print(
            f"📏 Chunk size: {self.chunk_size} lines with {self.overlap_lines} line overlap")
        ...
        print(
            f"Time: {elapsed:.1f}s ({len(all_chunks) / elapsed:.1f} chunks/sec)")
        print(
            f"Avg chunk size: "
            f"{sum(len(c['content']) for c in all_chunks) / len(all_chunks):.0f} chars")
        print(
            f"Storage: gs://{self.bucket_processed}/{repo_path}/chunks.jsonl")
        ...
        if language == 'python':
            ...
            for line in lines:
                if 'import' in line and 'from' in line:
                    ...
                    match = re.search(
                        r'(?:function|const|let|var)\s+(\w+)', line)
                    if match and len(context['functions']) < 25:
                        context['functions'].append(match.group(1))
    ...
    def _build_enriched_content(self, chunk: Dict, file_path: str,
        ...
        if file_context['imports']:
            parts.append(
                f"# Dependencies: {', '.join(file_context['imports'][:5])}")
        ...
        if file_context['functions']:
            parts.append(
                f"# Functions: {', '.join(file_context['functions'][:8])}")
    ...
    def _smart_line_chunk(self, content: str) -> List[Dict]:
        ...
        while i < len(lines):
            ...
            if chunk_end < len(lines) - 5:
                for j in range(chunk_end, max(
                        chunk_end - 20, chunk_start), -1):
                    line = lines[j].strip()
            ...
            if chunk_content.strip():
                summary = lines[chunk_start].strip(
                )[:100] if chunk_start < len(lines) else ""
            ...
            i = chunk_end - \
                self.overlap_lines if chunk_end < len(lines) else chunk_end
