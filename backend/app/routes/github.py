...
async def install_github_app(current_user: User = Depends(
        get_current_user)) -> RedirectResponse:
...
async def get_installed_repos(current_user: User = Depends(
        get_current_user)) -> list[GitHubRepo]:
