...
async def patch_user(update_data: UserUpdate, current_user: User = Depends(
        get_current_user)) -> UserRead:
...
async def get_workspaces(current_user: User = Depends(
        get_current_user)) -> list[Workspace]:
