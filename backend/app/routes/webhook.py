...
async def register_active_user(user_id: str, github_username: str, github_access_token: str,
    ...
    try:
        ...
        print(
            f"✓ Registered active user in Firestore: {github_username} (key: {key})")
...
async def find_active_user_with_repo_access(
        repo_full_name: str) -> Optional[dict]:
    """
        ...
        for session in active_users:
            ...
            try:
                ...
                if permissions.pull or permissions.push or permissions.admin:
                    ...
                    print(
                        f"     Permissions: push={permissions.push}, admin={permissions.admin}")
                    return session
                else:
                ...
                if e.status == 404:
                    print(
                        f"   ✗ {username} cannot access {repo_full_name} (404)")
                else:
...
async def _handle_push_event(
        payload: dict, background_tasks: BackgroundTasks) -> Dict:
    """
    ...
    commit_message = head_commit.get(
        "message", "push event") if head_commit else "push event"

    # Get author info
    commit_author = "unknown"
    if head_commit:
        author_data = head_commit.get("author", {})
        commit_author = author_data.get(
            "username") or author_data.get("name", "unknown")
    ...
    if not active_user:
        ...
        print(
            f"   Active users: {[u['github_username'] for u in all_sessions.get('users', [])]}")
    ...
    print(
        f"   Triggered by: {active_user['github_username']} (active collaborator)")
    print(f"   Pusher: {pusher_name}")
    print(f"   Commit: {commit_sha[:8]}")
    ...
    try:
        ...
        for session in active_users:
            ...
            try:
                ...
                # Verify they have at least pull access (collaborators
                # typically have this)
                if permissions.pull or permissions.push or permissions.admin:
                    print(f"   ✓ {username} has access!")
                    print(
                        f"     Permissions: pull={permissions.pull}, "
                        + "push={permissions.push}, admin={permissions.admin}")
                    return session
                else:
                ...
                if e.status == 404:
                    print(
                        f"   ✗ {username} cannot access "
                        f"{repo_full_name} (404 - not found "
                        f"or no access)"
                    )
                else:
                    print(
                        f"   ⚠️  GitHub API error for "
                        f"{username}: {e.status} - "
...
async def get_active_sessions_endpoint() -> Dict:
    ...
    result['note'] = (
        ...
        + "Any collaborator with repo access can trigger webhooks."
    )
