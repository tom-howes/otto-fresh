...
async def get_installation_token(
        installation_id: InstallationId) -> InstallationToken:
...
async def get_repository_details(
        installation_token: InstallationToken, owner: str, repository: str) -> GitHubRepo:
...
async def get_repository_contents(installation_token: InstallationToken,
                                  owner: str, repository: str, path: str = "") -> GitHubContent:
...
async def list_installation_repositories(
        installation_token: InstallationToken) -> list[GitHubRepo]:
    ...
    async with httpx.AsyncClient() as client:
        while True:
            url = (
                f"https://api.github.com/installation/"
                f"repositories?per_page={per_page}&page={page}"
            )
            response = await client.get(url, headers=headers)
            handle_error(response, status.HTTP_200_OK)
...
async def get_default_branch_sha(
        installation_token: InstallationToken, owner: str, repository: str, branch: str) -> SHA:
...
async def create_branch(installation_token: InstallationToken, owner: str,
                        repository: str, branch_name: str, sha: SHA) -> GitHubRef:
